...
function stat5InitToolbar() {
  ...
  const zoneBtn = stat5El("stat5-zone-toggle-btn");
  const zoneSide = stat5El("stat5-zone-side");
  if (zoneBtn && zoneSide) {
    zoneBtn.addEventListener("click", () => {
      const open = zoneSide.classList.toggle("open");
      zoneBtn.classList.toggle("active", open);
    });
  }
}
