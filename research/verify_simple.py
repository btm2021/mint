# ============================================================
# verify_simple.py — KIỂM CHỨNG ĐƠN GIẢN NHẤT
# Entry: fast flip CÙNG bias → market
# Exit : khi BIAS đảo (giữ trọn bias cycle)
# Hỏi : PnL có dương không?
# ============================================================
import sys, json, os
import numpy as np
import pandas as pd
...
for sym in ALL:
    ...
    for c in range(len(fast_cyc) - 1):
        ...
        ex = min(sc["end"], nn - 1)
        if ex <= cs: continue
        pnl = S * (bars[ex]["close"] / bars[cs]["close"] - 1) * 100
        rows.append(dict(sym=sym, pnl=pnl, hold=ex-cs))

df = pd.DataFrame(rows)
print(f"TỔNG LỆNH (cùng bias, giữ tới bias đảo): {len(df)}")
print(f"WIN: {(df.pnl>0).mean()*100:.1f}% | PnL TB {df.pnl.mean():+.3f}% | median {df.pnl.median():+.3f}% | Tổng {df.pnl.sum():+.0f}%")
print(f"Hold TB {df.hold.mean():.0f} nến")
print()
print("Theo nhóm volume:")
for grp, syms in GROUPS.items():
    sub = df[df.sym.isin(syms)]
    print(f"  {grp:4s} n={len(sub):4d} | WIN {(sub.pnl>0).mean()*100:5.1f}% | PnL TB {sub.pnl.mean():+6.3f}% | Tổng {sub.pnl.sum():+7.0f}%")
print()
print("Theo symbol:")
for sym in ALL:
    sub = df[df.sym == sym]
    if len(sub):
        print(f"  {sym:12s} n={len(sub):4d} | WIN {(sub.pnl>0).mean()*100:5.1f}% | PnL TB {sub.pnl.mean():+6.3f}% | Tổng {sub.pnl.sum():+7.0f}%")
