...
const withState = keptMeta.map((m, idx) => {
  ...
  const stPrev = h.states[Math.max(0, m.tIndex - 2)];
  const stPrev4 = h.states[Math.max(0, m.tIndex - 5)];
  return { m, st, stPrev, stPrev4 };
});
...
const combos = [];
{
  const rows = withState.filter((x) => x.st === 1 && x.stPrev === 1).map(({ m }) => simExit(m, g)).sort((a, b) => a.tEntry - b.tEntry);
  const s = stat(rows), so = stat(rows.filter((t) => t.tEntry >= A.oosSplit));
  if (s && so && s.n >= 100 && so.n >= 30) { combos.push({ allow: [1], label: "State1 + prev1", s, so }); console.log(`  ${("State1 + prev1").padEnd(26)} ${String(s.n).padStart(5)} ${s.win.toFixed(1).padStart(7)} ${s.exp.toFixed(3).padStart(8)} ${s.pf.toFixed(2).padStart(6)} ${s.mdd.toFixed(1).padStart(7)} ${String(so.n).padStart(5)} ${so.win.toFixed(1).padStart(9)} ${so.exp.toFixed(3).padStart(8)} ${so.pf.toFixed(2).padStart(6)}`); }
}
{
  const rows = withState.filter((x) => x.st === 1 && x.stPrev4 === 1).map(({ m }) => simExit(m, g)).sort((a, b) => a.tEntry - b.tEntry);
  const s = stat(rows), so = stat(rows.filter((t) => t.tEntry >= A.oosSplit));
  if (s && so && s.n >= 100 && so.n >= 30) { combos.push({ allow: [1], label: "State1 + prev4", s, so }); console.log(`  ${("State1 + prev4").padEnd(26)} ${String(s.n).padStart(5)} ${s.win.toFixed(1).padStart(7)} ${s.exp.toFixed(3).padStart(8)} ${s.pf.toFixed(2).padStart(6)} ${s.mdd.toFixed(1).padStart(7)} ${String(so.n).padStart(5)} ${so.win.toFixed(1).padStart(9)} ${so.exp.toFixed(3).padStart(8)} ${so.pf.toFixed(2).padStart(6)}`); }
}
