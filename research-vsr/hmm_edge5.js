...
class GaussianHMM {
  ...
  fit(obs, iters = 60, tol = 1e-4) {
    ...
    for (let it = 0; it < iters; it++) {
      ...
      if (it === 0) { const bn = []; outer: for (let t = 0; t < T; t++) for (let k = 0; k < K; k++) if (Number.isNaN(beta[t][k])) { bn.push({ t, k }); break outer; } if (bn[0]) { const t = bn[0].t; console.log("  [debug] beta NaN t=", t, "| beta[t+1]=", beta[t + 1], "| exp(logpdf)=", [0, 1, 2].map((j) => Math.exp(this.logpdf(obs[t + 1], j))), "| sigma=", this.sigma, "| mu=", this.mu.map((m) => m[0])); } }
