...
class GaussianHMM {
  ...
  fit(obs, iters = 60, tol = 1e-4) {
    ...
    let prevLL = -Infinity;
      const badObs = obs.findIndex((o) => !Number.isFinite(o[0]) || !Number.isFinite(o[1]));
      console.log("  [debug] badObs index:", badObs, "| mu0 khởi tạo:", this.mu.map((m) => m[0].toFixed(4)), "| sigma:", this.sigma.map((s) => s[0].toFixed(4)));
    for (let it = 0; it < iters; it++) {
      if (it === 0) console.log("  [debug fit] T=", T, "obs[0]=", obs[0]);
      ...
      for (let t = T - 2; t >= 0; t--) {
        for (let i = 0; i < K; i++) {
          ...
          if (it === 0 && Number.isNaN(beta[t][i])) { console.log("  [backward NaN] t=", t, "acc=", acc, "scale=", scale[t + 1], "beta[t+1]=", beta[t + 1]); return; }
        }
      }
      // gamma, xi
      ...
      const ll = scale.reduce((a, b) => a + Math.log(b), 0);
      if (it === 0) { console.log("  [debug] scale[0..3]=", scale.slice(0, 3), "| alpha[0]=", alpha[0], "| beta[0]=", beta[0], "| sigma trước M-step:", this.sigma.map((x) => x[0])); const bn = []; outer: for (let t = 0; t < T; t++) for (let k = 0; k < K; k++) if (Number.isNaN(beta[t][k])) { bn.push({ t, k }); break outer; } if (bn[0]) { let firstNaN = -1; for (let t = T - 1; t >= 0; t--) { if (Number.isNaN(beta[t][0])) { firstNaN = t; } } const t = firstNaN >= 0 ? firstNaN : bn[0].t; console.log("  [debug] beta NaN ĐẦU TIÊN (từ cuối): t=", firstNaN, "| beta[t]=", beta[firstNaN >= 0 ? firstNaN : 0], "| scale[t+1]=", scale[(firstNaN >= 0 ? firstNaN : 0) + 1], "| exp(logpdf) tại t+1:", [0, 1, 2].map((j) => Math.exp(this.logpdf(obs[(firstNaN >= 0 ? firstNaN : 0) + 1], j))), "| mu=", this.mu.map((m) => m[0].toFixed(3)), "| sigma=", this.sigma.map((x) => x[0].toFixed(6))); console.log("  [debug] beta NaN t=", t, "| beta[t+1]=", beta[t + 1], "| exp(logpdf)=", [0, 1, 2].map((j) => Math.exp(this.logpdf(obs[t + 1], j))), "| sigma=", this.sigma, "| mu=", this.mu.map((m) => m[0])); } }
