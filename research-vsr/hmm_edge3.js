...
class GaussianHMM {
  ...
  fit(obs, iters = 60, tol = 1e-4) {
    ...
    for (let it = 0; it < iters; it++) {
      ...
      if (it === 0) { const bn = []; outer: for (let t = 0; t < T; t++) for (let k = 0; k < K; k++) if (Number.isNaN(beta[t][k])) { bn.push({ t, k }); break outer; } console.log("  [debug] beta NaN đầu tiên:", JSON.stringify(bn[0]), "| scale[t+1]=", scale[bn[0] ? bn[0].t + 1 : 0], "| alpha[t]=", alpha[bn[0] ? bn[0].t : 0]); }
